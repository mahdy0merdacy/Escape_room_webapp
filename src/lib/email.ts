...
export function newBookingAdminEmail(params: {
  ...
  const textContent = `New booking received — #${ref}
...
Manage bookings: ${process.env.NEXTAUTH_URL ?? "https://elharba.tn"}/admin/bookings`;
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#111;border-radius:12px;overflow:hidden;border:1px solid #222;">
        ...
        <tr>
          <td style="background:#e11d48;padding:24px 32px;">
            ...
            <h1 style="margin:6px 0 0;font-size:22px;font-weight:900;color:#fff;">New Booking — #${ref}</h1>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:28px 32px 0;">
            <p style="margin:0 0 16px;color:#888;font-size:13px;">A new reservation was just confirmed.</p>
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#1a1a1a;border-radius:8px;margin-bottom:24px;">
              <tr><td style="padding:20px 24px;">
                ${row("Room", roomName)}
                ${row("Date", dateStr)}
                ${row("Time", `${startStr} – ${endStr}`)}
                ${row("Party size", `${partySize} people`)}
                <tr style="border-top:1px solid #333;">
                  <td style="padding:14px 0 0;color:#888;font-size:13px;width:120px;">Revenue</td>
                  <td style="padding:14px 0 0;font-size:18px;font-weight:800;color:#e11d48;">${total} TND</td>
                </tr>
              </td></tr>
            </table>
          </td>
        ...
        <tr>
          <td style="padding:0 32px 28px;">
            <p style="margin:0 0 12px;font-size:11px;letter-spacing:3px;text-transform:uppercase;color:#555;font-weight:600;">Customer</p>
        ...
        <tr>
          <td style="padding:0 32px 32px;">
            <table cellpadding="0" cellspacing="0"><tr><td style="background:#e11d48;border-radius:8px;">
              <a href="${process.env.NEXTAUTH_URL ?? "https://elharba.tn"}/admin/bookings"
                style="display:inline-block;padding:12px 28px;color:#fff;font-weight:700;text-decoration:none;font-size:14px;">
                View in Admin →
              </a>
            </td></tr></table>
          </td>
        ...
        <tr>
          <td style="padding:20px 32px;border-top:1px solid #222;text-align:center;">
            <p style="margin:0;color:#444;font-size:11px;">elharba admin notifications · ${adminEmail}</p>
          </td>
        </tr>
      </table>
    </td></tr>
